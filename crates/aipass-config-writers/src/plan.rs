...
pub fn plan_gemini_cli(home: &Path, entry: &ToolEntry) -> Result<(ConfigPlan, String)> {
    ...
    let mut content = format!(
        "# Generated by AIPass. Source this file before running Gemini CLI.\nexport GEMINI_API_KEY=\"$({})\"\n",
...
fn plan_grok_with_backend(
    ...
    let plan = new_plan(
        ToolId::Grok,
        target,
        format!("Configure Grok Build to use {}", entry.title),
...
fn aipass_config_id(entry: &ToolEntry) -> String {
    if let Some(secret_id) = &entry.secret_id {
        // Stable across display-name edits and distinct across keys and sites.
        return format!("aipass_{}_{}", entry.id.simple(), slug(secret_id));
    }
