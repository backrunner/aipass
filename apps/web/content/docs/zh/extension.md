...
Chrome 扩展可以将 API 密钥填充到 AI 服务商控制台，而密钥从不会存储在浏览器中。它通过 Chrome Native Messaging Host 与 AIPass 代理通信，每次填充都由已解锁保险库发出的短时授权来许可。

## 安装与配对

1. 从 Chrome 应用商店安装扩展，或从 [GitHub Releases](https://github.com/backrunner/aipass/releases) 下载扩展包，在 `chrome://extensions` 开启开发者模式后加载。
2. 注册 Native Messaging Host，让 Chrome 允许配对：
