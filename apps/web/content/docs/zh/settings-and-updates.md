---
title: 设置与更新
description: 桌面设置面板——锁定策略、密码与密钥轮换、同步、代理——以及自动更新行为。
navTitle: 设置与更新
order: 9
---
...
桌面应用的**设置**面板集中了保险库的所有运维操作。本页逐项介绍；发布订阅源的深入说明见[更新渠道](/docs/zh/update-channels)。
...
**设置 → 更新**显示当前版本、所选渠道（**官方**或 **Beta**）和手动的**立即检查**按钮。切换渠道会保存选择，并立即在新订阅源上重新检查。发现更新时可就地安装；应用前会验证签名。

应用还会在启动后不久自动检查，最多每 24 小时一次，并为新版本显示可关闭的横幅——关闭某个版本只会抑制该版本的提示。订阅源 URL 与渠道机制见[更新渠道](/docs/zh/update-channels)。
