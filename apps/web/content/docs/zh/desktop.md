...
成本估算使用的公共模型价格表独立于应用版本自动更新。后台 Agent 启动时会检查本地缓存，缺失或超过 24 小时就从 LiteLLM 下载；持续运行期间每天刷新，失败后每小时重试。离线或远端数据不可用时继续使用上次成功下载的价格表，没有有效缓存则使用内置价格。价格表下载无需解锁保险库，也无需重启代理。

自定义定价分组、手动价格和凭据倍率优先于公共价格表。历史用量在查询时重新估算，因此公共价格更新后，使用该价格表的历史成本也可能变化；这些金额是估算值，实际费用以服务商账单为准。
