...
The Chrome extension fills API keys into AI provider consoles without ever storing them in the browser. It talks to the AIPass agent through a Chrome Native Messaging host, and every fill is authorized by a short-lived grant from the unlocked vault.

## Install and pair

1. Install the extension from the Chrome Web Store, or load the package from [GitHub Releases](https://github.com/backrunner/aipass/releases) via `chrome://extensions` with developer mode.
2. Register the native messaging host so Chrome allows the pairing:

```bash
aipass native-host install --extension-id <chrome-extension-id>
```

The host name is `dev.aipass.native`. The installer writes the manifest (on macOS: `~/Library/Application Support/Google/Chrome/NativeMessagingHosts/dev.aipass.native.json`) with `allowed_origins` restricted to your extension ids, and saves the allowlist the native host enforces at runtime. Chromium, Edge, and Brave work with `--browser`. The desktop app shows the native host status and can repair the manifest from its extension settings; `aipass doctor` reports the same checks.
