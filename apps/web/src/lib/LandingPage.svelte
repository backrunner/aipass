<script lang="ts">
  ...
  const messages = {
    en: {
      ...
      detail: 'AIPass keeps your AI provider keys in an end-to-end encrypted vault on your machine. Choose how each tool connects: through the local proxy, a credential helper, or a native configuration file.',
      downloadCta: 'Download for macOS',
      docsCta: 'Read the docs',
      featureTitle: 'One vault for every AI workflow.',
      features: [
        ['Encrypted by default', 'Argon2id key derivation, XChaCha20-Poly1305 envelopes. Vault records and AIPass backups stay encrypted on disk.'],
        ['Configured in one step', 'Preview and write configurations for Codex, Claude Code, and Gemini CLI, with backups for rollback.'],
        ['Browser autofill, granted', 'The Chrome extension fills keys only with time-limited grants from the desktop app.']
      ],
      securityTitle: 'Encrypted storage. Clear access boundaries.',
      securityIntro: 'Each record is an encrypted envelope with its own data key, wrapped by a rotating vault epoch key. A recovery key protects against lockout. Tools and browser autofill can receive plaintext credentials; direct configuration modes may store them in the tool’s files.',
    ...
    zh: {
      ...
      headlineA: '所有 AI 密钥，',
      headlineB: '统一加密保管',
      detail: 'AIPass 将 AI 服务商的密钥保存在本机的端到端加密保险库中。按工具选择接入方式：本地代理、凭据助手或原生配置文件。',
      downloadCta: '下载 macOS 版',
      docsCta: '阅读文档',
      featureTitle: '一个保险库，覆盖所有 AI 工作流。',
      features: [
        ['默认加密', 'Argon2id 密钥派生 + XChaCha20-Poly1305 加密信封，保险库记录与 AIPass 备份在磁盘上保持加密。'],
        ['一步完成配置', '为 Codex、Claude Code、Gemini CLI 预览并写入配置，保留备份以便回滚。'],
        ['授权式浏览器填充', 'Chrome 扩展仅在桌面应用授权的时限内填充密钥。']
      ],
      securityTitle: '加密存储，清晰的访问边界。',
      securityIntro: '每条记录都是独立的加密信封，拥有随机的数据密钥，并由可轮换的保险库纪元密钥包裹。恢复密钥用于找回访问权限。工具和浏览器填充可以获得明文凭据；直接写入配置的模式可能将凭据保存在工具文件中。',
