...
const releaseTag =
  process.env.AIPASS_RELEASE_TAG ?? process.env.GITHUB_REF_NAME;
const version =
...
if (!releaseTag?.startsWith("v")) {
  throw new Error(
    "AIPASS_RELEASE_TAG or GITHUB_REF_NAME must be a v-prefixed release tag.",
  );
}
if (!version) {
...
const updaterArchives = bundleFiles
  .filter((file) => file.endsWith(".app.tar.gz"))
  .filter(
    (file) =>
      !requireUniversal || file.includes(`${sep}universal-apple-darwin${sep}`),
  )
  .sort();

if (updaterArchives.length === 0) {
  const qualifier = requireUniversal ? " universal" : "";
  throw new Error(
    `No${qualifier} macOS updater archive (*.app.tar.gz) found under target/*/release/bundle.`,
  );
}

const updaterArchive = await selectBundleFile(updaterArchives);
const currentBundleRoot = bundleRoot(updaterArchive);
const currentBundleFiles = bundleFiles.filter((file) =>
  file.startsWith(currentBundleRoot),
);
const dmgFiles = currentBundleFiles
  .filter((file) => file.endsWith(".dmg"))
  .sort();
const signatureFile = `${updaterArchive}.sig`;
if (!bundleFiles.includes(signatureFile)) {
  throw new Error(
    `Missing updater signature next to ${relativeToRepo(updaterArchive)}.`,
  );
}
if (dmgFiles.length === 0) {
  throw new Error(
    `No macOS DMG artifact found under ${relativeToRepo(currentBundleRoot)}.`,
  );
}
...
console.log(
  `Prepared macOS release artifacts in ${relativeToRepo(artifactDir)}.`,
);

async function listBundleFiles() {
  const candidates = [
    join(repoRoot, "target"),
    join(repoRoot, "apps", "desktop", "src-tauri", "target"),
  ];
  const files = [];
  ...
  return files.filter((file) =>
    file.includes(`${sep}release${sep}bundle${sep}`),
  );
}
...
function bundleRoot(file) {
  ...
  if (index === -1)
    throw new Error(`File is not under a release bundle directory: ${file}`);
